...
def create_grid(locked_pos={}):  # *
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            print("this is a diction",locked_pos)
            if (j, i) in locked_pos:
                print("second",locked_pos[(j,i)])
                c = locked_pos[(j,i)]
                print("2222222222222222",c)
                grid[i][j] = c
                print("3333333333333333",grid)
    return grid
...
def main(win):
    ...
    while run:
        ...
        cv2.imshow("BALANCE CAM", cv2.resize(image, (640, 480)))
        cv2.waitKey(10)
